  ...
  <div class="container-fluid">
	...
	<div id="wrapper" style="margin-left: 60px;">
		...
		<div id="head">
			...
			<br>
			<form name="f" action="WebsiteQuery" class="fm" >
		...
		<div id="main">
					...
					<table cellpadding="0" cellspacing="0" class="result">
						<tr>
							<td class=f>
								...
								</h3> <font size=-1><div class="realtime">来源：&nbsp;${article.source}&nbsp;</div>
									${article.text}<br> <font color="#008000"><span><a
											class="g" href="${article.url}">${article.url}</a>...&nbsp;${article.postTime}&nbsp;</span></font>-&nbsp;
									<a href="${article.url}"
			...
			<p id="page">
				<c:if test="${page.totalRecords!=0}">
					<a href='WebsiteQuery?query=${query}&pageNo=0'
						class="n">首页</a>
					<c:if test="${page.pageNo != 0}">
						<a
							href='WebsiteQuery?query=${query}&pageNo=${page.getPreviousPageNo()}'
							class="n">&lt;上一页</a>
					</c:if>
					<c:forEach var="item" begin="${page.getBegin()}"
						...
						<c:choose>
							<c:when test="${item!=page.pageNo}">
								<a href='WebsiteQuery?query=${query}&pageNo=${item}'>
					...
					<c:if test="${page.pageNo != page.getTotalPages()-1}">
						<a
							href='WebsiteQuery?query=${query}&pageNo=${page.getNextPageNo()}'
							class="n">下一页&gt;</a>
					</c:if>
					<a
						href='WebsiteQuery?query=${query}&pageNo=${page.getBottomPageNo()}'
						class="n">最后一页</a>
				</c:if>
			</p>
		</div>
